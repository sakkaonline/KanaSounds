...
class MainActivity : AppCompatActivity() {
    private var mMenuLanguage = 0
    private var mVoiceType = 0
    private var mLearningCharacter = 0
    private val mMenuLanguageList = listOf("English", "ひらがな", "漢字", "Ro-ma Ji")
    private var mVoiceTypeList = listOf<String>()
    private var mLearningCharacterList = listOf<String>()
    ...
    private fun setSpinners(){
        ...
        mLearningCharacterSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(parent3: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                changeCharacter(mLearningCharacter)
            }
        }
        Log.d("KanaSounds", "resetSpinners() done")
    }

    private fun changeMenuLanguage(i: Int) {
        mMenuLanguage = i
        val settings = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
    ...
    private fun changeVoiceType(i: Int) {
        mVoiceType = i
        val settings = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val editor = settings.edit()
        editor.putInt(VoiceTypeKEY, mVoiceType)
        editor.commit()
        Log.d("KanaSounds", "changeVoiceType() done")
    }

    private fun changeCharacter(i: Int) {
        mLearningCharacter = i
        val settings = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val editor = settings.edit()
        editor.putInt(LearningCharacterKEY, mLearningCharacter)
        editor.commit()
        Log.d("KanaSounds", "changeCharacter() done")
    }
